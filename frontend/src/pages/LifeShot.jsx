...
const LifeShot = () => {
  ...
  const handleDragEnd = (result) => {
    ...
    if (destinationId.startsWith("frame-")) {
      const frameIndex = parseInt(destinationId.split("-")[1], 10);
  ...
  return (
    <DragDropContext onDragEnd={handleDragEnd}>
      <div className="lifeshot-page">
        ...
        >
          <LifeShotFrame selectedColor={selectedColor} frameImages={frameImages} />
                    ...
                    <Draggable key={index} draggableId={`image-${index}`} index={index}>
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={`lifeshot-image-item ${snapshot.isDragging ? "dragging" : ""}`}
                        >
        ...
        <div className="button-wrapper">
          ...
          <button onClick={handleCapture} className="chat-button">
            저장하기
          </button>
        </div>
      </div>
