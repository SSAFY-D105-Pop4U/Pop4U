...
//                CannedAccessControlList.PublicRead));
    ...
    public void deleteS3(String filePath) throws Exception {
        try {
            ...
            if (amazonS3Client.doesObjectExist(bucket, key)) {
                ...
                S3Uploader.log.info("File deleted successfully: {}", key);
            } else { // file not found
                S3Uploader.log.warn("File not found: {}", key);
                log("파일이 존재하지 않습니다.");
            }
        } catch (Exception exception) {
            S3Uploader.log.info(exception.getMessage());
        }
        S3Uploader.log.info("[S3 Uploader]: S3에 있는 파일 삭제");
    }
    ...
    private void removeNewFile(File targetFile){
        ...
        if (targetFile.delete()){
            S3Uploader.log.info("[파일 업로드]: "+name + "로컬 파일 삭제 완료");
            return;
        }
        S3Uploader.log.info("[파일 업로드]: "+name + "로컬 파일 삭제 실패");
    }
}
